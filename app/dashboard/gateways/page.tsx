...
import { GatewayTransactionsView } from "@/components/views/finances/gateway-transactions-view"
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (isAuthenticated && user) {
        ...
        if (profileError) {
          console.error("GatewaysDashboardPage: Erro ao buscar perfil do usuário:", profileError)
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center">
        ...
        <p className="ml-2">Carregando dashboard de gateways... ({pageStatus})</p>
      </div>
    )
  }
