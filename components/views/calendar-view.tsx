...
function VLine({ left, height, z = 60 }: { left: number; height: number; z?: number }) {
  return (
    <div className="absolute top-0 pointer-events-none" style={{ left, height, zIndex: z }}>
...
function NowLine({ top, left, right = 0, z = 80 }: { top: number; left: number; right?: number; z?: number }) {
  return (
    <div className="absolute pointer-events-none" style={{ top, left, right, zIndex: z }}>
...
export default function CalendarView() {
  ...
  const handleCreateEvent = () => {
    ...
    const e = new Date(base); e.setHours(10, 0, 0)
  ...
  const handleSaveEvent = () => {
    ...
    if (isCreatingEvent) {
      ...
      setEvents(events.map((ev) =>
        ev.id === selectedEvent.id ? { ...ev, title: newEvent.title || ev.title, start: s, end: e, description: newEvent.description, location: newEvent.location, color: safeColor, allDay: newEvent.allDay } : ev,
      ))
